...
def numerical_model(T, input_data, X_train, Y_train, X_valid, Y_valid,i):
        ...
        # 콜백함수
        mc = ModelCheckpoint("./상주_토마토_모델파일_AIC/상주_토마토_모델파일_AIC_"+i+".h5", save_best_only=True,monitor='val_loss')
        # 모델구성
...
def binary_model(T, input_data, X_train, Y_train, X_valid, Y_valid,i):
        ...
        # 콜백함수
        mc = ModelCheckpoint("./상주_토마토_모델파일_AIC/상주_토마토_모델파일_AIC_"+i+".h5", save_best_only=True,monitor='val_accuracy')
        # 모델구성
        model = Sequential([LSTM(16, input_shape=[T, input_data.shape[1]], activation='relu')])
        #repeat vector
        model.add(RepeatVector(1))
...
if __name__ == "__main__":
    ...
    df  = pd.read_csv('D:/theimz/조석준/NAVER_WORKS/스마트팜_혁신밸리/22년12월/모델고도화코드개발/스마트팜_혁신밸리_고도화_모델코드(전달)/상주_온실별_작기별_데이터/상주_경영_토마토_1_5_iqr_이상치대체_10분단위_1220_ver1.csv')
    ...
    with tensorflow.device("/device:GPU:0"):
        ...
        for i in custom_x.keys():
            ...
            # 스케일러 저장
            dump(scaler, open('./상주_토마토_스케일러_AIC/상주_토마토_스케일러_AIC_'+i+'.pkl', 'wb'))
            ...
            #모델 불러오기
            model = tensorflow.keras.models.load_model('D:/theimz/조석준/NAVER_WORKS/스마트팜_혁신밸리/22년12월/모델고도화코드개발/스마트팜_혁신밸리_고도화_모델코드(전달)/상주_온실별_작기별_데이터/상주_토마토_모델파일_AIC/상주_토마토_모델파일_AIC_'+i+'.h5')
        ...
        상주_범주형_성능지표.to_csv('상주_토마토_범주형_성능지표.csv',index=False, header = True)
        상주_연속형_성능지표.to_csv('상주_토마토_연속형_성능지표.csv',index=False, header = True)